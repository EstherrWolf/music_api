...
import { CustomRequest } from '../types/request.type';
  ...
  export class ExceptionHandlerFilter implements ExceptionFilter {
    constructor(private readonly systemLogger: SystemLogger) {}
      ...
      const statusCode = exceptionResponse.statusCode || exception.getStatus?.() || HttpStatus.INTERNAL_SERVER_ERROR;
      /* ------------- Get exception response infomation  -------------*/
      ...
      const handleErrorResponse = (
        ...
        response.status(statusCode).json({ statusCode, errorMessage, data });
      };
      const handlers: {
        condition: boolean;
        type: ErrorLogType;
      ...
      for (const { condition, type, errorMessage, data } of handlers) {
        if (condition) {
          return handleErrorResponse(type, statusCode, errorMessage, data);
        }
      }
